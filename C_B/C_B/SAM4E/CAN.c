...
int CAN_Getmsg(uint8_t *data)
{
	...
	for (pcan_fifo->Head;pcan_fifo->Head<=pcan_fifo->Tail;)
	{
		*data=pcan_fifo->Buff[pcan_fifo->Head++];
		CAN_Buff[CAN_msg_lengh++]=*data;
		if (pcan_fifo->Head==pcan_fifo->Tail)
		{
			CAN_msg_status = 1;
			break;
		}
...
void CAN_ADCval_receive(void)
{
	CAN0->CAN_IER=CAN_IER_MB6;
	
	//수신 사서함 설정
	can0_mailbox.ul_mb_idx = 6;
	can0_mailbox.uc_obj_type = 1;
	can0_mailbox.ul_id_msk = CAN_MAM_MIDvA_Msk;
...
void CAN_BoardID_send()
{
	...
	can0_mailbox.ul_datah = 0x12345678;
