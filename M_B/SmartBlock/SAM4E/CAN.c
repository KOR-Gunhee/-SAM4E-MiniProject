...
ISR_ST_CAN_FIFO volatile *pcan_fifo=0;
...
extern void can_mailbox_init(Can *p_can, can_mb_conf_t *p_mailbox)
{
	uint8_t uc_index;
...
extern uint32_t can_mailbox_read(Can *p_can, can_mb_conf_t *p_mailbox)
{
	uint32_t ul_status;
	uint8_t uc_index;
	uint32_t ul_retval;
...
extern uint32_t can_init(Can *p_can, uint32_t ul_mck, uint32_t ul_baudrate)
{
	uint32_t ul_flag;
	uint32_t ul_tick;
...
int CAN_Getmsg(uint8_t *data)
{
	...
	if(ISR_Can_Buff.Head==ISR_Can_Buff.Tail)return 0;
	pcan_fifo=&ISR_Can_Buff;
	szbuff=ISR_SZ_CAN_BUFF;
	
	if(pcan_fifo->Head>=szbuff){pcan_fifo->Head=0;}
	
	BD_CH=pcan_fifo->Buff[pcan_fifo->Head++];
...
void CAN_msgTx(int ch, uint32_t id)
{
//	CAN_IDreset(ch);
	CAN_ID(ch,id);
	CAN_TX(ch);
...
void CAN_BoardID_receive(void)
{
	CAN0->CAN_IER=CAN_IER_MB4;

	//수신 사서함 설정
	can0_mailbox.ul_mb_idx = 4;
	can0_mailbox.uc_obj_type = 1;
	can0_mailbox.ul_id_msk = CAN_MAM_MIDvA_Msk;
	can0_mailbox.ul_id = CAN_MID_MIDvA(0x10);
	can_mailbox_init(CAN0, &can0_mailbox);
}

void CAN_CH_receive(uint16_t id)
{
	CAN0->CAN_IER=CAN_IER_MB5;
	
	//수신 사서함 설정
	can0_mailbox.ul_mb_idx = 5;
	can0_mailbox.uc_obj_type = 1;
	can0_mailbox.ul_id_msk = CAN_MAM_MIDvA_Msk;
	can0_mailbox.ul_id = CAN_MID_MIDvA(id);
	can_mailbox_init(CAN0, &can0_mailbox);
}

void Main_Init_send(void)
{
	//송신 사서함 설정
	can0_mailbox.ul_mb_idx = 0;
	can0_mailbox.uc_obj_type = 3;
	can0_mailbox.uc_tx_prio = 15;
	can0_mailbox.uc_id_ver = 0;
	can0_mailbox.ul_id_msk = 0;
	can_mailbox_init(CAN0, &can0_mailbox);

	can0_mailbox.ul_id = CAN_MID_MIDvA(0x01);
...
void CAN_ADCval_send(void)
{
	//송신 사서함 설정
	can0_mailbox.ul_mb_idx = 2;
	can0_mailbox.uc_obj_type = 3;
	can0_mailbox.uc_tx_prio = 15;
	can0_mailbox.uc_id_ver = 0;
	can0_mailbox.ul_id_msk = 0;
	...
	can0_mailbox.ul_id = CAN_MID_MIDvA(0x20);
	can0_mailbox.ul_datal = 0x00;
	can0_mailbox.ul_datah = 0x87654321;
	can0_mailbox.uc_length = 8;
	can_mailbox_write(CAN0, &can0_mailbox);
